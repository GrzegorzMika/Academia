...
\begin{figure}
<<echo = FALSE, fig = TRUE>>=
rcauchy(100) %>% enframe() %>% ggplot(aes(x = value)) + geom_density(fill = 'blue') +
  theme_classic() + labs(title = "Rozkład Cauch'ego")
@
\end{figure}
\end{zd}
...
df %>%
  ...
  ggplot(aes(x = value)) + geom_density(fill = 'green') + theme_classic() + labs(title = 'CTG - to działa!')
@
\end{figure}
\end{zd}
